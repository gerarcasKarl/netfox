...
//  netfox_ios
//
//  Created by karl gerarcas on 23/01/2020.
//  Copyright © 2020 kasketis. All rights reserved.
//

//import Foundation
import UIKit
import CommonCrypto // <CommonCrypto/CommonCrypto.h>
        ...
        DispatchQueue.global(qos: DispatchQoS.QoSClass.default).async { () -> Void in
            ...
            if let publicKeyRef = self.getPublicKeyReference() {
                ...
                DispatchQueue.main.async(execute: { () -> Void in
                    ...
                    cipherText.deinitialize(count: 0)
                })
                return
...
//                    plainText.deinitialize()
                    plainText.deinitialize(count: 0)
                })
                return
            } else { DispatchQueue.main.async(execute: { completion(false, nil, .keyNotFound) }) }
        }
